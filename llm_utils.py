...
def load_model(model_name: str, quantize_4bit: bool = False, device_map: str = "auto") -> tuple:
    ...
    if quantize_4bit:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            model_name, quantization_config=bnb_cfg, device_map=device_map, trust_remote_code=True)
    else:
        model = AutoModelForCausalLM.from_pretrained(
...
def generate(model, tokenizer, messages: list[dict], max_new_tokens: int = 512,
    ...
    # apply_chat_template returns BatchEncoding on some transformers versions — handle both
    ...
    with torch.no_grad():
        output_ids = model.generate(
            ...
            do_sample=do_sample, pad_token_id=tokenizer.eos_token_id, use_cache=True)
