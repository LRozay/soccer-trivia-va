...
- For "generate_trivia": output a trivia QUESTION ending with "?" — do NOT reveal the answer. Example: "Which Portuguese forward has won the Champions League 5 times and the Ballon d'Or 5 times?"
- For "generate_quiz": output numbered questions only, each ending with "?" — do NOT reveal answers.
- For "solve_clue": name the player and explain why they match the clue.
- For "check_answer": confirm correct or incorrect and explain.
- For "get_hint": give just the hint, do not reveal the answer.
- For "current_events": form a trivia question from the search results.
- For "general_chat": respond warmly and helpfully.
...
You will be given:
  ...
  3. A draft response (a trivia question)

Your job: verify the FACTS in the draft are accurate based on the player data provided.
- If the draft is a valid trivia question with correct facts, reply ONLY with: APPROVED
- If there is a factual error, reply with: CORRECTED: <fixed question>
- NEVER reveal the answer in your response.
- NEVER rewrite a question as an explanation.
Be brief."""
...
def self_reflect(
    ...
    if reflection.strip() == "APPROVED" or reflection.startswith("APPROVED"):
        return draft  # always keep the original question on approval
