...
HARDER_RE   = re.compile(r"\b(harder|more difficult|increase difficulty|tougher)\b", re.I)
EASIER_RE   = re.compile(r"\b(easier|simpler|too hard|make it easy)\b", re.I)
TRIVIA_RE   = re.compile(r"\b(trivia|question|riddle|quiz)\b", re.I)
...
def pre_classify(message: str, session_state: dict) -> dict | None:
    ...
    # Difficulty escalation — regenerate at new difficulty
    if HARDER_RE.search(lower):
        return _parsed("generate_trivia", difficulty="hard")
    if EASIER_RE.search(lower):
        return _parsed("generate_trivia", difficulty="easy")
    ...
    if has_active and not TRIVIA_RE.search(lower):
        ...
        if is_short or looks_guess or ends_q:
...
def dispatch_tools(parsed: dict, session_state: dict) -> dict:
    ...
    if intent in ("generate_trivia", "generate_quiz"):
        ...
        query = parsed.get("web_query") or "soccer Champions League latest news"
        tools_used.append("web_search")
        raw_results = web_search(query, max_results=4)
        results["web_results"] = raw_results
        # Extract usable snippets for the LLM
        snippets = [r.get("body", "")[:200] for r in raw_results if r.get("body")]
...
CURRENT_EVENTS_SYSTEM = (
    GUARD + "\n\n"
    "You are given recent soccer news snippets. "
    "Using ONLY the facts in these snippets, write ONE trivia question "
    "that has a clear, verifiable answer. "
    "Format: just the question, nothing else. End with '?'"
)
...
RESPOND_SYSTEM = (
    ...
    "For get_explanation: explain why the player fits the question.\n"
...
def generate_response(user_message: str, parsed: dict, tool_results: dict,
    ...
    # ── Current events: separate focused prompt ───────────────
    if intent == "current_events":
        snippets = tool_results.get("snippets", [])
        if not snippets:
            return "I couldn't find recent soccer news. Try asking for a database trivia question instead!"
        combined = " | ".join(snippets[:3])
        messages = [
            {"role": "system", "content": CURRENT_EVENTS_SYSTEM},
            {"role": "user",   "content": f"News snippets: {combined}"},
        ]
        question = generate(model, tokenizer, messages,
                            max_new_tokens=100, temperature=0.5, model_size=model_size)
        # Store as active question (no DB player, so no answer checking)
        return f"🌐 **Current events trivia:**\n\n{question}"

    # ── LLM for solve_clue, explanation, general_chat ────────
    trimmed = {k: v for k, v in tool_results.items()
               if k not in ("tools_used", "web_results", "snippets")}
...
def run_pipeline(user_message: str, session_state: dict, model, tokenizer,
    ...
    # Step 1a: fast rule-based
    parsed = pre_classify(user_message, session_state)

    # Step 1b: LLM only when needed
    if parsed is None:
